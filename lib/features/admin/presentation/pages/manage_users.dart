...
class UserProfile {
  ...
  // ✅ الإصلاح: DocumentSnapshot بدلاً من QueryDocumentSnapshot
...
class _ManageUsersPageState extends ConsumerState<ManageUsersPage> {
  ...
  late PaginationHelper<UserProfile> _pagination;
  bool _isInitialLoad = true;

  @override
  void initState() {
    super.initState();
    _pagination = PaginationHelper<UserProfile>(
      baseQuery: FirebaseFirestore.instance
          .collection('users')
          .orderBy('createdAt', descending: true),
      fromDoc: UserProfile.fromDoc,
      pageSize: 15,
    );
    _loadInitial();
                            ...
                            OutlinedButton.icon(
                              ...
                              onPressed: () async {
                                try {
                                  ...
                                  _snack(
                                    'تم إرسال رابط إعادة التعيين إلى ${emailCtrl.text}',
                                    success: true,
                                  );
                                } catch (e) {
  ...
  Widget build(BuildContext context) {
    // ✅ الإصلاح: حذف ref.watch(_usersStreamProvider) — نعتمد على _pagination فقط

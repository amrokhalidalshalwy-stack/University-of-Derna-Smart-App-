...
class FeesPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(l10n.feesTitle), centerTitle: true),
      body: auth.when(
        data: (user) {
          if (user == null) {
            return Center(child: Text(l10n.pleaseLogin));
          }
          final async = ref.watch(feeRecordsProvider(user.uid));
          return async.when(
            data: (records) {
              return RefreshIndicator(
                onRefresh:
                    () async => ref.invalidate(feeRecordsProvider(user.uid)),
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      _buildBalanceCard(context, records),
                      const SizedBox(height: 32),
                      Align(
                        alignment: AlignmentDirectional.centerEnd,
                        child: Text(
                          l10n.feesLocalRecord,
                      ...
                      if (records.isEmpty)
                        EmptyStateWidget(
                          ...
                          title: l10n.feesEmpty,
                          subtitle: l10n.pullToRefreshSync,
                          actionLabel: l10n.refreshAction,
                          onAction:
                              () =>
                                  ref.invalidate(feeRecordsProvider(user.uid)),
                        )
                      else
                        ...records.map(
                          (r) => _buildTransactionItem(
                            context,
                            r.title.isEmpty ? l10n.feesDefaultTitle : r.title,
                            '${r.amount} ${r.currency}',
                            r.dueDate ?? '-',
  ...
  Widget _buildBalanceCard(BuildContext context, List<FeeRecord> records) {
    final l10n = AppLocalizations.of(context)!;
    return Container(
      ...
      child: Column(
        children: [
          Text(
            l10n.feesSavedRecords,
            style: TextStyle(color: Theme.of(context).colorScheme.onPrimary.withValues(alpha: 0.7), fontSize: 14),
          ),
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Text(
              isPaid ? l10n.feesPaid : l10n.feesUnpaid,
              style: TextStyle(
